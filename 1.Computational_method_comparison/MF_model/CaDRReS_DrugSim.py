...
if __name__ == "__main__":
    ...
    fps = [Chem.RDKFingerprint(x) for x in train_mol_rdkit]

    start = time.time()
    mol_sim_mat = np.zeros((len(train_mol), len(train_mol)))
    for i in range(len(train_mol)):
        if (i+1)%100 == 0:
            print ("{} of {} ({:.2f})s".format(i+1, len(train_mol), time.time()-start))
            start = time.time()
        for j in range(len(train_mol)):
            sim_score = DataStructs.FingerprintSimilarity(fps[i], fps[j])
            mol_sim_mat[i, j] = sim_score
    mol_sim_df = pd.DataFrame(mol_sim_mat, columns=prism_train.index.tolist(), index=prism_train.index.tolist())

    # based on drug similarity
    # kernel feature based only on training samples
    drug_sample_list = prism_train.index.tolist()
    X_train = mol_sim_df
    print(X_train.shape)
    # observed drug response
    Y_train = prism_train.loc[drug_sample_list]
    print(Y_train.shape)
    ...
    cadrres_model_dict, cadrres_output_dict = train_model(Y_train, X_train, Y_train, X_train, 10, 0.0, 150000, 0.01, model_spec_name="cadrres-wo-sample-bias", save_interval=5000, output_dir=output_dir)
    # saving model
    print('Saving ' + output_dir + '{}_param_dict.pickle'.format("cadrres-wo-sample-bias_drugSim"))
    pickle.dump(cadrres_model_dict, open(output_dir + '{}_param_dict.pickle'.format("cadrres-wo-sample-bias_drugSim"), 'wb'))
    print('Saving ' + output_dir + '{}_output_dict.pickle'.format("cadrres-wo-sample-bias_drugSim"))
    pickle.dump(cadrres_output_dict, open(output_dir + '{}_output_dict.pickle'.format("cadrres-wo-sample-bias_drugSim"), 'wb'))